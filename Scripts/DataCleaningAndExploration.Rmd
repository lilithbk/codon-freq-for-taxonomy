...
df <- read_csv("../codon+usage/codon_usage.csv", show_col_types = FALSE)
...
# Save plot
ggsave("Plots/MeanCodonFreq.pdf")
```
...
Need a validation set to train neural networks without biasing results on the test set, so will use the same sets of data to train, validate, and test all models. Will use a 60/20/20 train/validation/test split.
...
test_id <- sample(1:nrow(df), nrow(df) * 0.2)
df.test <- df[test_id, ]
dim(df.test)
# Update df to avoid overlap between the three sets
df <- df[-test_id, ]

# Set aside validation set
## 20% less data in df now, so need 25% of what is remaining to be 20% of the overall observations
val_id <- sample(1:nrow(df), nrow(df) * 0.25)
df.val <- df[val_id, ]
dim(df.val)
## Remaining observations are for training
df.train <- df[-val_id, ]
dim(df.train)

# Save each data frame in its own CSV
write_csv(df.test, "Data/set_splits/test_codons.csv")
write_csv(df.val, "Data/set_splits/val_codons.csv")
write_csv(df.train, "Data/set_splits/train_codons.csv")
```
